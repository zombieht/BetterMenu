...
### v2.0.7 (2026-06-09)
- **结构重构**：整理并优化项目的文件组织结构，将源文件分类整理至 Models、Services 和 Views 目录中，提升代码可读性与可维护性。
- **稳定性修复**：修复了系统重启后会自动打开主界面的问题。
...
- **稳定性修复**：优化终端拉起 AppleScript 脚本逻辑，解决在终端未启动时会偶发开启两个窗口（一个默认窗口、一个执行 `cd` 窗口）的问题。
- **线程安全**：修复 `ExternalAppLauncher` 中拉起外部应用时的并发线程安全隐患，将 App 激活逻辑分发至主线程执行。
